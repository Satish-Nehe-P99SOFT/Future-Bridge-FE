...
export async function generateStaticParams() {
  const whitePapersPath = path.join(process.cwd(), "public", "data", "White Paper");
  try {
    ...
    console.error("[generateStaticParams - White Papers] Error reading directory:", error);
    return [];
  }
}
...
async function getWhitePaperData(slug: string): Promise<WhitePaperData | null> {
  const filePath = path.join(process.cwd(), "public", "data", "White Paper", `${slug}.json`);
  try {
    const fileContent = await fs.readFile(filePath, "utf-8");
    const jsonData = JSON.parse(fileContent);
    // Use the correct dataKey "whitePapers"
    const whitePaperData: WhitePaperData | undefined = jsonData?.data?.whitePapers?.[0];
    return whitePaperData || null;
  } catch (error) {
    console.error(`[getWhitePaperData] Error reading/parsing file ${filePath}:`, error);
    ...
    <article className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
      ...
      {imageAttrs?.url && (
        <div className="mb-8 md:mb-12 relative aspect-video rounded-lg overflow-hidden shadow-lg">
          <Image
            src={imageAttrs.url}
            alt={imageAttrs.alternativeText || title || "White paper featured image"}
            fill
      ...
      <div className="prose prose-lg max-w-none">
        {content.length > 0 ? (
          content.map((block, index) => <BlockRenderer key={index} block={block} />)
        ) : (

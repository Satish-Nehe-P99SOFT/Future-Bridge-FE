...
export async function generateStaticParams() {
  ...
  try {
    ...
    console.error("[generateStaticParams - Webinars] Error reading directory:", error);
    return [];
  }
}
...
async function getWebinarData(slug: string): Promise<WebinarData | null> {
  const filePath = path.join(process.cwd(), "public", "data", "Webinars", `${slug}.json`);
  try {
    ...
    console.error(`[getWebinarData] Error reading/parsing file ${filePath}:`, error);
    ...
    <article className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
      ...
      {imageAttrs?.url && (
        <div className="mb-8 md:mb-12 relative aspect-video rounded-lg overflow-hidden shadow-lg">
          <Image
            src={imageAttrs.url}
            alt={imageAttrs.alternativeText || title || "Webinar featured image"}
            fill

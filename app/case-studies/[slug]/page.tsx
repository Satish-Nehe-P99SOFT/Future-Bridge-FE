...
export async function generateStaticParams() {
  ...
  const caseStudiesPath = path.join(process.cwd(), "public", "data", "Case Study");
  try {
    ...
    console.error("[generateStaticParams - Case Studies] Error reading directory:", error);
    return [];
  }
}
...
async function getCaseStudyData(slug: string): Promise<CaseStudyData | null> {
  const filePath = path.join(process.cwd(), "public", "data", "Case Study", `${slug}.json`);
  try {
    const fileContent = await fs.readFile(filePath, "utf-8");
    const jsonData = JSON.parse(fileContent);
    // Use the correct dataKey "caseStudies"
    const caseStudyData: CaseStudyData | undefined = jsonData?.data?.caseStudies?.[0];
    return caseStudyData || null;
  } catch (error) {
    console.error(`[getCaseStudyData] Error reading/parsing file ${filePath}:`, error);
    ...
    <article className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
      ...
      {imageAttrs?.url && (
        <div className="mb-8 md:mb-12 relative aspect-video rounded-lg overflow-hidden shadow-lg">
          <Image
            src={imageAttrs.url}
            alt={imageAttrs.alternativeText || title || "Case study featured image"}
            fill

...
import { OurSolutionsBlock } from "@/types/blocks";
...
const findBlock = <T extends { __typename: string }>(
  ...
  return blocks.find((block) => block && block.__typename === typeName) as T | undefined;
};

export default async function Home() {
  // Fetch data for all sections concurrently
  const [rawEvents, rawWebinars, rawClientStories, solutionsData, perspectivesData, aboutData] =
    await Promise.all([
  ...
  const validClientStories = rawClientStories.filter((story) => story && story.attributes);
...
const ourSolutionsBlock = findBlock<OurSolutionsBlock>(
  solutionsData?.blocks || [],
  "ComponentBlocksOurSolutions"
);
  ...
  return (
    <>
      ...
      <SolutionsSection block={ourSolutionsBlock} />

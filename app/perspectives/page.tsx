    ...
    try {
      ...
      const fileReadPromises = jsonFiles.map(async (fileName) => {
        ...
        try {
          ...
          const itemData: JsonItemData | undefined = jsonData?.data?.[dirInfo.dataKey]?.[0];
          ...
          if (itemData?.title && itemData?.slug) {
            ...
            const perspectiveItem: PerspectiveItem = {
              ...
              imageAlt: itemData.featuredImage?.alternativeText ?? itemData.title ?? undefined, // Use ?? undefined
            };
            return perspectiveItem;
          } else {
            console.warn(`Skipping ${fileName}: Missing title, slug, or expected data structure.`);
            return null;
          }
        } catch (parseError) {
          console.error(`Error reading or parsing file ${filePath}:`, parseError);
          return null;
        }
      });
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="container mx-auto px-4 pb-16">
        {allItems.length > 0 ? (
          ...
          <p className="text-center text-gray-500 py-12">No perspective items found.</p>
        )}
      </div>
    </div>
  );

...
export async function generateStaticParams() {
  ...
  try {
    ...
    console.error("[generateStaticParams - Articles] Error reading directory:", error);
...
async function getArticleData(slug: string): Promise<ArticleData | null> {
  ...
  try {
    ...
    console.error(`[getArticleData] Error reading/parsing file ${filePath}:`, error);
    ...
    <article className="container mx-auto px-4 py-8 md:py-12 max-w-4xl">
      ...
      {imageAttrs?.url && (
        <div className="mb-8 md:mb-12 relative aspect-video rounded-lg overflow-hidden shadow-lg">
          <Image
            src={imageAttrs.url}
            alt={imageAttrs.alternativeText || title || "Article featured image"}
            fill

...
const SidebarContentAndReferenceSection: React.FC<SidebarContentProps> = ({ block }) => {
  const {
    title,
    description,
  ...
  return (
    <section className={`my-8 ${containerClasses}`}>
      {title && <h3 className="text-xl font-semibold mb-4 text-gray-800 border-b pb-2">{title}</h3>}
      {description && (
        <div className={contentWrapperClasses} dangerouslySetInnerHTML={{ __html: description }} />
      )}
      {!title && !description && (
        <p className="text-gray-500 italic">Sidebar/Reference section content missing.</p>
      )}
    </section>
  );
};

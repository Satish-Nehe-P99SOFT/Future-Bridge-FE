...
const ClientsStories: React.FC<ClientsStoriesProps> = ({ block }) => {
  ...
  const renderStory = (story: ClientStory, index: number) => {
    ...
    const storyCard = (
      <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-full">
        ...
        <div className="border-t border-gray-200 p-4 flex items-center">
          {imageAttrs?.url && (
            <div className="flex-shrink-0 mr-4">
              <div className="relative h-12 w-12 rounded-full overflow-hidden">
                <Image
                  src={imageAttrs.url}
                  alt={imageAttrs.alternativeText || story.clientName || "Client"}
                  fill
          ...
          <div>
            {story.clientName && <p className="font-semibold text-gray-800">{story.clientName}</p>}
            {story.position && <p className="text-sm text-gray-500">{story.position}</p>}
          </div>
        </div>
      </div>
    );
    ...
    if (hasLink) {
      return story.isExternal ? (
        <a key={index} href={story.link!} target={target} rel={rel} className="block h-full">
          {storyCard}
        </a>
      ) : (
        <Link key={index} href={story.link!} target={target} className="block h-full">
          {storyCard}
        </Link>
      );
    }
  ...
  return (
    <div className="my-12 px-4">
      ...
      {subHeading && (
        <p className="text-gray-600 text-center mb-8 max-w-3xl mx-auto">{subHeading}</p>
      )}

...
const IndustryParticipantsBlock: React.FC<IndustryParticipantsBlockProps> = ({ block }) => {
  // Combine the main item (if present) with the list items for rendering
  const allItems: IndustryItem[] = [];
  if (block.industryParticipantsButtonName && block.industryParticipantsButtonLink) {
    allItems.push({
  ...
  return (
    <div className="my-12">
      {block.heading && <h2 className="text-3xl font-bold text-center mb-4">{block.heading}</h2>}
      {block.subHeading && (
        <p className="text-lg text-center text-gray-600 mb-10">{block.subHeading}</p>
      )}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {allItems.map((item, index) => {
          ...
          return item.isExternal ? (
            <a key={index} href={item.buttonLink || "#"} target={linkTarget} rel={rel}>
              {linkContent}
            </a>
          ) : (

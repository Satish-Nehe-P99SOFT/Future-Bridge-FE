...
const StandaloneButtonBlock: React.FC<StandaloneButtonBlockProps> = ({ block }) => {
  ...
  const isInternalPageLink = !block.isExternal && block.buttonLink?.startsWith("/");
  ...
  return (
    <div className="my-6 text-center">
      ...
      {isInternalPageLink ? (
        <Link href={block.buttonLink || "#"} target={linkTarget} className={buttonClasses}>
          {buttonContent}
        </Link>
      ) : (
        <a href={block.buttonLink || "#"} target={linkTarget} rel={rel} className={buttonClasses}>
          {buttonContent}
        </a>
      )}
    </div>

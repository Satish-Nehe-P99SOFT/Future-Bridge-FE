...
import type { BoBlock, QuestionItem, BoSolutionItem, BusinessFunctionItem } from "@/types/blocks";
...
const filterBySection = (items: (QuestionItem | BoSolutionItem)[], sectionKey: string) => {
...
const filterQuestions = (questions: QuestionItem[], keyword: string): QuestionItem[] => {
  // Keywords match the section names in the data (e.g., Insights_Short_Term)
  const lowerKeyword = keyword.toLowerCase();
  return questions.filter((q) => q.sections.toLowerCase().includes(lowerKeyword));
};
...
const BoBlockComponent: React.FC<BoBlockProps> = ({ block }) => {
  ...
  const activeFunctionData = functions.length > 0 ? functions[activeIndex] : null;
  ...
  if (functions.length === 0) {
    ...
    return (
      <div className="my-12 p-6 bg-gray-100 border border-gray-300 rounded-lg text-center">
        <p className="text-gray-600">BoBlockComponent: No business function data found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="my-12 py-10 bg-white">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {subHeading && (
          <p className="text-lg text-gray-600 text-center max-w-3xl mx-auto mb-10">{subHeading}</p>
        )}
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-start gap-12 lg:gap-16">
          ...
          <div className="relative w-full max-w-[400px] lg:max-w-[450px] shrink-0">
            <svg viewBox={`0 0 ${svgSize} ${svgSize}`} className="w-full h-auto">
                  ...
                  >
                    ...
                    >
                      ...
                      {func.title.split("&").map((part, i) => (
                        <tspan key={i} x={labelPos.x} dy={i > 0 ? "1.2em" : "0"}>
                          {part.trim()}
                        </tspan>
                      ))}
                    </text>
              ...
              {/* Central Circle */}
              <circle cx={center} cy={center} r={innerRadius * 0.8} className="fill-white" />
              <text
                x={center}
                y={center}
                textAnchor="middle"

...
const PostContent2ColumnLayout: React.FC<PostContent2ColumnLayoutProps> = ({ block }) => {
  ...
  const renderItem = (item: PostItem, index: number) => {
    ...
    const content = (
      <div className="bg-white rounded-lg shadow-md overflow-hidden group flex flex-col h-full">
        ...
        <div className="p-4 flex flex-col flex-grow">
          ...
          <h3 className="text-lg font-semibold mb-2 text-gray-800 flex-grow">{item.title}</h3>
          {item.date && <p className="text-sm text-gray-500 mb-3">{item.date}</p>}
          {item.excerpt && (
    ...
    if (item.link) {
      return item.isExternal ? (
        <a key={index} href={linkHref} target={linkTarget} rel={rel} className="block h-full">
          {content}
        </a>
      ) : (
        <Link key={index} href={linkHref} target={linkTarget} className="block h-full">
          {content}
        </Link>
      );
    } else {
  ...
  return (
    <div className="my-12">
      ...
      {/* 2-column grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">{items.map(renderItem)}</div>
    </div>
  );
};

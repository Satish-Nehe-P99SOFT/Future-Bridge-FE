...
const QuotesAndContentsBlockComponent: React.FC<QuotesAndContentsProps> = ({ block }) => {
  const { quote, content, quoteVariant } = block;

  // Determine layout direction based on variant
  const isQuoteLeft = !quoteVariant || !quoteVariant.toLowerCase().includes("right");
  ...
  return (
    ...
    >
      ...
      {quote && (
        <aside className="w-full md:w-1/3 lg:w-1/4 flex-shrink-0">
          <blockquote className="relative p-6 bg-blue-50 border-l-4 border-blue-500 rounded-r-lg shadow-md">
            ...
            <p className="text-lg italic text-blue-800 leading-relaxed z-10 relative">{quote}</p>
          </blockquote>
        </aside>
      )}
      ...
      {!quote && !content && <p className="text-gray-500 italic">Quote and content missing.</p>}
    </div>
  );
};

        ...
        {platformLinks.length > 0 && (
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {platformLinks.map((link, index) => {
              ...
              const target = link.isExternal ? "_blank" : link.target || "_self";
              const rel = link.isExternal ? "noopener noreferrer" : undefined;
              const isInternalPageLink = !link.isExternal && linkHref.startsWith("/");
              ...
              return (
                <React.Fragment key={index}>
                  {isInternalPageLink ? (
                    ...
                    <a href={linkHref} target={target} rel={rel} className={buttonClasses}>
                      {link.buttonName}
                    </a>
                  )}
                </React.Fragment>

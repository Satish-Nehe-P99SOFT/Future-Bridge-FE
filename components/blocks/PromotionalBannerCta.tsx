...
const PromotionalBannerCta: React.FC<PromotionalBannerCtaProps> = ({ block }) => {
  const {
    heading,
    subHeading,
    buttonName,
  ...
  const sizeClass = PromotionBannerSize === "Thin" ? "py-8 md:py-12" : "py-16 md:py-20";
  ...
  const contentWrapperClasses = cn("container mx-auto px-6 relative z-10", textAlignmentClass);
  ...
  const primaryButtonClasses = "bg-white text-blue-600 hover:bg-gray-100 focus:ring-white";
  const secondaryButtonClasses =
    "bg-transparent border border-white hover:bg-white hover:bg-opacity-10 focus:ring-white";
  const buttonVariantClass =
    buttonVariant?.toLowerCase() === "secondary" ? secondaryButtonClasses : primaryButtonClasses;
  ...
  return (
    ...
    >
      ...
      {bgImageUrl && <div className="absolute inset-0 bg-black bg-opacity-40 z-0"></div>}

      <div className={contentWrapperClasses}>
        {heading && <h2 className="text-2xl md:text-3xl font-bold mb-3">{heading}</h2>}
        {subHeading && (
          <p className="text-base md:text-lg opacity-90 max-w-2xl mx-auto">{subHeading}</p>
        )}
        {renderButton()}
      </div>
    </div>
